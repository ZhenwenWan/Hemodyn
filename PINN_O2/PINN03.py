# PINN02.py — time-dependent 1D–2D coupled oxygen transport with PINNs (DeepXDE)
# Changes:
# - Added robust "save artifacts" block (checkpoints + metadata + predictions).
# - Added a proper pde_residual operator used by RAR to avoid NameError.
# - Added a best-checkpoint callback during Adam warmup.
...
def main():
    ...
    ckpt = dde.callbacks.ModelCheckpoint(
        filepath=os.path.join(outdir, "oxygen_pinn"),
        save_better_only=True,
        monitor="train loss",
        period=200,
    )
    losshist, train_state = model.train(
        iterations=4000, display_every=200, callbacks=[resampler, earlyA, ckpt]
    )

    # Phase B: switch to L-BFGS for fast convergence on physics loss
    try:
        from deepxde.optimizers import config as lbfgs_cfg
        lbfgs_cfg.set_LBFGS_options(maxiter=300, maxcor=20, ftol=1e-9, gtol=1e-7, maxls=20)
    ...
    # --------- RAR operator (fix for NameError) ----------
    # PyTorch predict(operator=...) expects fn(inputs, outputs) and returns a tensor.
    # We build a closure that mirrors the PDE and returns the pointwise residual norm.
    def make_pde_residual(vars_dict, params_true, y_eps):
        P_var, Dt_var, Vmax_var, Km_var = vars_dict["P"], vars_dict["Dt"], vars_dict["Vmax"], vars_dict["Km"]
        u, Dc, R, beta = params_true.u, params_true.Dc, params_true.R, params_true.beta

        def I_of_t_torch(t):
            return params_true.I0 + params_true.I1 * torch.sin(2 * torch.pi * params_true.f_m * t)

        def op(inputs, outputs):
            x = inputs[:, 0:1]
            y = inputs[:, 1:2]
            t = inputs[:, 2:3]
            Oc = outputs[:, 0:1]
            ...
            # Derivatives
            Oc_t = dde.grad.jacobian(Oc, inputs, i=0, j=2)
            Ot_t = dde.grad.jacobian(Ot, inputs, i=0, j=2)
            Oc_x  = dde.grad.jacobian(Oc, inputs, i=0, j=0)
            Oc_xx = dde.grad.hessian(Oc, inputs, i=0, j=0)
            Ot_xx = dde.grad.hessian(Ot, inputs, i=0, j=0)
            Ot_yy = dde.grad.hessian(Ot, inputs, i=0, j=1)

            # Reaction & residuals
            Veff = Vmax_var * (1.0 + beta * I_of_t_torch(t)**2)
            reac = Veff * Ot / (Km_var + Ot + 1e-12)

            res_t = Ot_t - Dt_var * (Ot_xx + Ot_yy) + reac
            sink  = (2.0 * P_var / R) * (Oc - Ot)
            res_c_raw = Oc_t + u * Oc_x - Dc * Oc_xx + sink
            mask = torch.exp(- (y / y_eps)**2)
            res_c = mask * res_c_raw

            # Return scalar residual per point
            return torch.sqrt(res_c**2 + res_t**2)

        return op

    pde_residual = make_pde_residual(vars_dict, params_true, y_eps_used)
    ...
    for rar_round in range(1):  # a few refinement rounds
        ...
        Xpool = geom.random_points(pool_n)
        res   = model.predict(Xpool, operator=pde_residual)  # now defined
        res   = np.abs(np.squeeze(res))

        k = min(new_n, len(res))
        worst_idx = np.argpartition(res, -k)[-k:]
        worst_idx = np.unique(worst_idx[worst_idx < len(Xpool)])

        print(f"RAR round {rar_round}: mean residual = {res.mean():.3e}; adding {len(worst_idx)} anchors")

        model.data.add_anchors(Xpool[worst_idx])

        early = dde.callbacks.EarlyStopping(min_delta=1e-4, patience=2000)
        model.compile("adam", lr=1e-3, external_trainable_variables=list(vars_dict.values()))
        model.train(iterations=10000, disregard_previous_best=True, callbacks=[early])
        model.compile("L-BFGS", external_trainable_variables=list(vars_dict.values()))
        model.train()
    ...
    # DeepXDE's save() appends "-<iteration>.<ext>" (PyTorch -> .pt) and stores both
    # model_state_dict and optimizer_state_dict. Use restore(path, device=...) to load.  [docs]
    # (We'll save once at the end; best-checkpointed files are already written during Adam.)
