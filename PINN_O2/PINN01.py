...
def build_inverse_pinn(params_true: TrueParams,
    ...
    net = dde.nn.FNN([3] + [96] * 5 + [2], "tanh", "Glorot uniform")  # (x,y,t)->(Oc,Ot)
...
def main():
    ...
    losshist, train_state = model.train(iterations=8000, display_every=1000, callbacks=[resampler, earlyA])
    ...
    for rar_round in range(3):  # a few refinement rounds
        Xpool = geom.random_points(40000)
        # Use current model to evaluate PDE residual at Xpool
        f_res = model.predict(Xpool, operator=model.data.pde)
        err = np.abs(f_res).mean()
        print(f"RAR round {rar_round}: mean residual = {err:.3e}")
        # Add top-K worst points
        K = 200
        worst_idx = np.argsort(np.abs(f_res).ravel())[-K:]
        model.data.add_anchors(Xpool[worst_idx])
        # Short Adam fine-tune with resampling, then brief LBFGS
        model.compile(optimizer="adam", lr=5e-4, external_trainable_variables=list(vars_dict.values()))
        model.train(iterations=3000, callbacks=[dde.callbacks.PDEPointResampler(period=200)])
        model.compile(optimizer="L-BFGS", external_trainable_variables=list(vars_dict.values()))
        model.train()
